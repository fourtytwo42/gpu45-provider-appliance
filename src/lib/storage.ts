...
export async function getStorageInventory(): Promise<{ root: string; items: StorageItem[]; usedBytes: number; reclaimableBytes: number }> {
  ...
  const fixed = [
    ...
    ["/models/music/ace-step", "music-models", "production", true, "ACE-Step Music Studio profiles"],
    ["/models/music/levo2", "music-models", "production", true, "LeVo 2 AMD Music Studio profile"],
    ["/models/music/jobs", "music-outputs", "protected", true, "Music Studio inputs and generated outputs"],

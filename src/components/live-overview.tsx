...
export function LiveOverview({ initial }: { initial: DashboardSnapshot; endpoint: { allowAnonymous: boolean; visibleModels: number; endpointBase: string } }) {
  ...
  useEffect(() => { const source = new EventSource("/api/events?topics=telemetry,jobs"); source.addEventListener("telemetry", (event) => { const next = JSON.parse((event as MessageEvent).data) as LiveTelemetry; setLive(next); setCharts((current) => updateCharts(current, next)); }); source.addEventListener("jobs", (event) => setJobs(JSON.parse((event as MessageEvent).data) as JobsPayload)); return () => source.close(); }, []);

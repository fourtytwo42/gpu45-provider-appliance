...
import { ArchiveRestore, HardDrive, LoaderCircle, Play, ShieldCheck } from "lucide-react";
...
export function BackupPanel({ initial }: { initial: BackupStatus }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-md border border-[#223044] bg-[#0d131c] p-3 text-xs text-[#8a98aa]"><div className="font-mono text-[#cbd5e1]">{status.repository}</div><div className="mt-1">Last finished: {status.finishedAt ? new Date(status.finishedAt).toLocaleString("en-US", { timeZone: "America/Chicago" }) : "never"}</div></div>
      {!status.externalConfigured ? <div className="flex gap-2 rounded-md bg-[#121a26] p-3 text-sm text-[#cbd5e1]"><HardDrive className="mt-0.5 h-4 w-4 shrink-0 text-[#36fba1]" /><span>Local encrypted backup configured with daily, weekly, and monthly retention.</span></div> : <div className="flex gap-2 text-sm text-emerald-200"><ShieldCheck className="h-4 w-4" />Local and secondary backup targets configured.</div>}

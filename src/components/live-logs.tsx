...
export function LiveLogs({ initialLines }: { initialLines: string[] }) {
  ...
  return <section className="border border-white/10 bg-[#0a1119]">
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 p-4"><div><h1 className="text-lg font-semibold text-white">Service journals</h1><p className="mt-1 text-xs text-slate-500">Live systemd output from the bare-metal appliance.</p></div><div className="flex items-center gap-2 text-[11px] text-slate-600"><span className="h-2 w-2 bg-emerald-400" />updated {updatedAt.toLocaleTimeString("en-US", { timeZone: "America/Chicago" })}</div></div>

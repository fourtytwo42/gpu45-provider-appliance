    ...
    <div className="grid gap-5 2xl:grid-cols-[minmax(0,0.9fr)_minmax(30rem,1.1fr)]">
      <SectionCard title="Compose" description="The selected model determines which workflows and controls are available.">
        <form onSubmit={(event) => void submit(event)} className="space-y-5">
          <div className="grid grid-cols-4 gap-1 rounded-lg bg-[#0a1018] p-1">{MODES.map(({ id, label, icon: Icon }) => <button key={id} type="button" aria-label={label} onClick={() => chooseMode(id)} className={cn("flex min-h-10 items-center justify-center gap-2 rounded-md px-2 text-xs transition", mode === id ? "bg-[#20283a] text-[#e9d5ff]" : "text-[#718096] hover:text-white")}><Icon className="h-3.5 w-3.5" /><span className="hidden sm:inline">{label}</span></button>)}</div>

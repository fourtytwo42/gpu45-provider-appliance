...
export function KeysManager({ initialKeys, initialAllowAnonymous }: { initialKeys: ApiKeyRecord[]; initialAllowAnonymous: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="border border-white/10 bg-[#0a1119] p-4"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold text-white">Issued keys</h2><span className="font-mono text-xs text-slate-500">{keys.length} total</span></div><div className="mt-3 divide-y divide-white/10 border-y border-white/10">{keys.length === 0 ? <div className="py-8 text-center text-sm text-slate-500">No managed keys yet. Anonymous access is {allowAnonymous ? "enabled" : "disabled"}.</div> : keys.map((key) => { const expired = key.expiresAt ? new Date(key.expiresAt) <= new Date() : false; const blocked = Boolean(key.suspendedAt) || expired; return <div key={key.id} className="grid gap-3 py-4 lg:grid-cols-[1.1fr_.9fr_1fr_auto] lg:items-center"><div><div className="flex items-center gap-2"><span className="font-medium text-white">{key.name || "Unnamed key"}</span><span className={`border px-1.5 py-0.5 text-[10px] uppercase ${blocked ? "border-rose-400/30 text-rose-300" : "border-emerald-400/30 text-emerald-300"}`}>{expired ? "expired" : key.suspendedAt ? "suspended" : "active"}</span></div><div className="mt-1 font-mono text-xs text-slate-500">{key.keyPrefix}</div><div className="mt-1 text-[11px] text-slate-600">Expires {key.expiresAt ? new Date(key.expiresAt).toLocaleString() : "never"}</div></div><div className="grid grid-cols-3 gap-3 text-xs"><div><div className="font-mono text-white">{key.requestCount.toLocaleString()}</div><div className="text-slate-600">requests</div></div><div><div className="font-mono text-white">{key.promptTokens.toLocaleString()}</div><div className="text-slate-600">input</div></div><div><div className="font-mono text-white">{key.completionTokens.toLocaleString()}</div><div className="text-slate-600">output</div></div></div><div className="text-xs text-slate-400">{key.models.length ? key.models.map((model) => <div key={model.model} className="flex justify-between gap-3"><span className="truncate">{model.model}</span><span className="font-mono text-slate-600">{model.requests} req</span></div>) : <span className="text-slate-600">No usage recorded</span>}<div className="mt-1 text-[11px] text-slate-600">Last used {key.lastUsedAt ? new Date(key.lastUsedAt).toLocaleString() : "never"}</div></div><div className="flex gap-1">{key.suspendedAt ? <button title="Reactivate key" onClick={() => void suspend(key.id, false)} className="p-2 text-emerald-300 hover:bg-emerald-400/10"><Play className="h-4 w-4" /></button> : <button title="Suspend key" onClick={() => void suspend(key.id, true)} className="p-2 text-amber-300 hover:bg-amber-400/10"><Ban className="h-4 w-4" /></button>}<button title="Delete key" onClick={() => void remove(key.id)} className="p-2 text-rose-300 hover:bg-rose-400/10"><Trash2 className="h-4 w-4" /></button></div></div>; })}</div></section>
  </div>;
}
